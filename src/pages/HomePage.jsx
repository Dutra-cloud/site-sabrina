...
const HomePage = () => {
    ...
    return (
        <div className="space-y-6 md:space-y-12">
            {/* Hero Section */}
            <section className="relative min-h-[280px] sm:min-h-[400px] md:min-h-[600px] rounded-2xl md:rounded-3xl overflow-hidden flex items-center px-4 md:px-20 shadow-xl md:shadow-2xl py-8 md:py-20">
            ...
            {/* WhatsApp CTA Section - Compact Mobile */}
            <section className="max-w-6xl mx-auto px-2 md:px-4">
                <div className="bg-[var(--bg-secondary)] rounded-xl md:rounded-2xl border border-green-500/20 p-4 md:p-10 flex flex-col md:flex-row items-center gap-4 md:gap-8 shadow-md hover:shadow-green-500/10 transition-shadow duration-300">
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        <div className="flex-shrink-0 bg-green-100 dark:bg-green-900/30 p-2 md:p-4 rounded-full">
                            <svg viewBox="0 0 24 24" className="w-8 h-8 md:w-12 md:h-12 text-green-600 dark:text-green-500 fill-current" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="w-full md:w-auto flex flex-col gap-2">
                        <a
                            ...
                            className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 px-6 rounded-full transition-colors shadow-md hover:shadow-lg text-sm md:text-base w-full md:w-auto"
                        >
                            <MessageCircle size={18} />
                            Entrar no Grupo
                        </a>
                        <p className="md:hidden text-[10px] text-center text-[var(--text-secondary)] flex items-center justify-center gap-1">
                            <span>🚚</span> Entregas em Sertãozinho e região
                        </p>
                    </div>
                </div>
            </section>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8">
                    {categories.map((category) => (
                        <Link
                            ...
                            className="group relative rounded-2xl md:rounded-3xl overflow-hidden aspect-[4/3] border-2 border-transparent hover:border-sky-500/50 shadow-lg hover:shadow-2xl hover:shadow-sky-500/20 transition-all duration-500"
                        >
                            ...
                            {/* Content */}
                            <div className="absolute inset-0 flex flex-col justify-end p-4 md:p-8 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                                <h3 className="text-white font-black text-lg md:text-4xl mb-1 md:mb-2 tracking-tight drop-shadow-lg group-hover:text-sky-400 transition-colors">
                                    {category.name}
                                </h3>
                                <div className="w-8 md:w-12 h-1 md:h-1.5 bg-sky-500 rounded-full transform scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500 delay-100"></div>
                                <p className="text-gray-300 text-xs md:text-sm font-medium mt-2 md:mt-3 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500 delay-100">
                                    Ver produtos &rarr;
                                </p>
                            </div>
                        </Link>
